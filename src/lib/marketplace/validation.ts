  ...
  for (const license of licenses) {
    if (!groups[license.id]) {
      ...
      console?.printWarning(
        'Duplicate License found at API border',
        JSON.stringify({ licenseA: license, licenseB: groups[license.id][0] }, null, 2)
      );
      console?.printWarning(
        'Using latest updated (other license will be ignored)',
        JSON.stringify({ data: latestLicense.data }, null, 2)
      );
...
export function assertRequiredLicenseFields(license: License) {
  ...
  validateField(
    license,
    (license) => license.data.partnerDetails?.billingContact,
    (o) => !o || typeof o.email === 'string'
  );
}
...
export function assertRequiredTransactionFields(transaction: Transaction) {
  ...
  validateField(
    transaction,
    (transaction) => transaction.data.purchasePrice,
    (n) => typeof n === 'number'
  );
  validateField(
    transaction,
    (transaction) => transaction.data.vendorAmount,
    (n) => typeof n === 'number'
  );
  ...
  validateField(
    transaction,
    (transaction) => transaction.data.partnerDetails?.billingContact,
    (o) => !o || typeof o.email === 'string'
  );
}

function validateField<T extends Transaction | License, V>(
  o: T,
  accessor: (o: T) => V,
  validator: (o: V) => boolean = (o) => !!o
) {
  const val = accessor(o);
  const path = accessor.toString().replace(/^(\w+) => /, '');
  if (!validator(val))
    throw new AttachableError(
      `Missing field (in ${o.id}): ${path} (found ${JSON.stringify(val)})`,
