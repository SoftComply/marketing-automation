...
export function updateContactsBasedOnMatchResults(engine: Engine, allMatches: RelatedLicenseSet[]) {
  for (const license of allMatches) {
    const contacts = new Set(
      license
        ...
        .map((license) => engine.pipedrive.personManager.getByEmail(license.data.technicalContact!.email)!)
    );

    flagPartnersViaCoworkers([...contacts]);

    for (const contact of contacts) {
      const items = [...license, ...license.flatMap((l) => l.transactions)];
   ...
   */
  for (const contact of engine.pipedrive.personManager.getAll()) {
...
function setPartnerDomainFor(partnerDomains: Set<string>, records: (License | Transaction)[]) {
  for (const record of records) {
    const contactsToCheck = [record.partnerContact, record.billingContact, record.techContact].filter(isPresent);

    record.partnerDomain =
      contactsToCheck
        .flatMap((c) => c.allEmails)
        .map(domainFor)
        .find((domain) => partnerDomains.has(domain)) ?? null;
  }
}
