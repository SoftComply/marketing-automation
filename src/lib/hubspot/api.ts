...
export default class HubspotAPI {
  ...
  public async downloadHubspotEntities<D>(entityAdapter: EntityAdapter<D>) {
    ...
    const apiProperties = [
      ...typedEntries(entityAdapter.data)
        .map(([k, v]) => v.property)
        .filter(isPresent),
      ...entityAdapter.additionalProperties,
    ];

    const associations = inputAssociations.length > 0 ? inputAssociations : undefined;
    ...
    try {
      ...
      return entities.map(({ id, properties, associations }) => ({
        id,
        properties,
        associations: Object.entries(associations || {}).flatMap(([, { results }]) =>
          results.map((item) => `${item.type}:${item.id}` as RelativeAssociation)
        ),
      }));
    } catch (e: any) {
      ...
      if (
        ...
        throw new Error(
          `Failed downloading ${entityAdapter.kind}s.\n  Response body: ${JSON.stringify(body)}\n  Error stacktrace: ${e.stack}`
        );
      }
    }
  }
  ...
  public async createEntities(kind: EntityKind, entities: NewEntity[]): Promise<ExistingEntity[]> {
    ...
    await this.batchUpsert(
      ...
      entities,
      async (batch) =>
        await this.apiFor(kind)
          .batchApi.create({ inputs: batch })
          .then(
  ...
  private async batchUpsert<T, U>(
    kind: EntityKind,
    entities: T[],
    fn: (array: T[]) => Promise<unknown>
  ): Promise<void> {
  ...
  private apiFor(kind: EntityKind) {
    switch (kind) {
      ...
      case 'contact':
        return this.client.crm.contacts;
    }
  }
}
