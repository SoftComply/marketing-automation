...
export class DealDataLogger {
    ...
    for (const action of actions) {
      switch (action.type) {
        ...
        case 'noop': {
          ...
          if (action.deal) {
            const dealId = action.deal.id;
            const { value, dealStage } = action.deal.data;
            const recordId = [...action.deal.getMpacIds()].join(',');
            const stage = DealStage[dealStage];
            details += ` deal=${dealId}, record=${recordId}, stage=${stage}, amount=${value}`;
          }
  ...
  logRecords(records: (License | Transaction)[]) {
    ...
    const ifTx = (fn: (r: Transaction) => string) => (r: License | Transaction) =>
